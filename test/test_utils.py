...
def test_unzip_data(tmp_path: Path):
    ...
    data = unzip_data(
        input_data=DEMO_DATA_FOLDER.joinpath("R02_DatiCPA_1991.zip"), output_folder=tmp_path
    )
    print(data)
    assert isinstance(data, Path)
...
def test_get_census_dictionary():
    ...
    target_year = 2021
    data = get_census_dictionary(census_year=target_year, region_list=[3, 5])
...
def test_csv_from_excel_no_metadata(mock_open_workbook, mock_tqdm, tmp_path: Path):
    ...
    rows_data = [["Header1", "Header2"], ["Data1", "Data2"]]
